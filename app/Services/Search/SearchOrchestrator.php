...
class SearchOrchestrator extends \App\Services\BaseService implements SearchServiceInterface
{
    ...
    public function searchQuery(SearchQuery $query): SearchResult
    {
        $result = match ($query->scope) {
            'admin' => $this->searchAdmin($query->q, $query->limit, $query->offset),
            'user' => $query->actorId ? $this->searchUser($query->q, $query->actorId, $query->limit, $query->offset) : [],
    ...
    {
        return $this->adminProvider->quickSearchAds($q, $userId, $limit);
    }

    public function quickSearchSubmissions(string $q, ?int $userId, int $limit): array
    {
        return $this->adminProvider->quickSearchSubmissions($q, $userId, $limit);
    }
}
