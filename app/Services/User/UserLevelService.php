...
class UserLevelService extends \App\Services\BaseService
{
    ...
    [
        ...
        'ref_id' => null,
        'idempotency_key' => \Core\IdempotencyKey::generateFromPayload('level_purchase', [
            'user_id' => $userId,
            'level_slug' => $levelSlug,
            'date' => date('Y-m-d') // اجازه خرید یکبار در روز طبق لاجیک بیزینسی
        ]),
    ]
);
...
$txId = $withdrawResult['transaction_id'] ?? null;
    ...
    public function changeLevel(int $userId, ?string $fromSlug, string $toSlug, string $changeType, string $reason = ''): bool
    {
        ...
        if (in_array($changeType, ['upgrade', 'purchase', 'admin'])) {
            try {
                \Core\EventDispatcher::getInstance()->dispatch('level.upgraded', new \App\Events\LevelUpgradedEvent(
                    $userId,
                    $fromSlug ?? 'none',
                    $toSlug,
                    $changeType . ': ' . $reason
