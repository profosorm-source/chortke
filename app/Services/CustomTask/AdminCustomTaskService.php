...
class AdminCustomTaskService extends BaseService
{
    ...
    public function approveTask(int $taskId, int $adminId): array
    {
        try {
            ...
            if ($task->status !== 'pending' && $task->status !== 'pending_review') {
                return ['success' => false, 'message' => 'وضعیت تسک قابل تغییر نیست.'];
            }

            $this->db->beginTransaction();
            $this->db->query("UPDATE ads SET status = 'active', updated_at = NOW() WHERE id = ?", [$taskId]);
            $this->db->commit();
    ...
    public function rejectTask(int $taskId, int $adminId, ?string $reason): array
    {
        try {
            ...
            if ($task->status === 'rejected' || $task->status === 'completed') {
                return ['success' => false, 'message' => 'تسک قبلا رد یا تکمیل شده است.'];
            }
            ...
            if ($remaining > 0) {
                ...
                if (!$txId) {
                    $this->db->rollBack();
                    return ['success' => false, 'message' => 'خطا در بازگشت بودجه به کیف پول.'];
                }
            }

            $this->db->query("UPDATE ads SET status = 'rejected', remaining_budget = 0, updated_at = NOW() WHERE id = ?", [$taskId]);
            $this->db->commit();
    ...
    public function pauseTask(int $taskId, int $adminId): array
    {
        try {
            ...
            if ($task->status !== 'active') {
                return ['ok' => false, 'message' => 'تسک فعال نیست.'];
            }

            $this->db->beginTransaction();
            $this->db->query("UPDATE ads SET status = 'paused', updated_at = NOW() WHERE id = ?", [$taskId]);
            $this->db->commit();
