...
EOT;
    ...
    public function createInvestment(int $userId, array $data): array
    {
        ...
        try {
            ...
            $txUuid = 'tx_' . bin2hex(random_bytes(16));
            // ثبت تراکنش برداشت
            $withdrawalTransaction = $this->db->query(
                "INSERT INTO transactions (user_id, transaction_id, type, currency, amount, balance_before, balance_after, status, description, request_id, created_at)
                 VALUES (?, ?, 'withdraw', 'usdt', ?, ?, ?, 'pending', 'سرمایه‌گذاری جدید', ?, NOW())",
                [$userId, $txUuid, $amount, $balanceBefore, $balanceAfter, bin2hex(random_bytes(16))]
            );

            // به‌روزرسانی موجودی
            $updated = $this->db->query(
                "UPDATE wallets SET balance_usdt = ? WHERE user_id = ?",
                [$balanceAfter, $userId]
            );
    ...
    public function applyProfitLossToBatch(array $investmentIds, int $tradingRecordId, float $profitLossPercent, string $period, int $adminId): void
    {
        ...
        foreach ($investmentIds as $invId) {
            $inv = $this->investmentModel->find((int)$invId);
