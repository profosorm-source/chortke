...
extends \App\Services\BaseService
{
    ...
    public function batchInsert(string $table, array $records): array
    {
        ...
        try {
            ...
            $sql = "INSERT INTO {$table} ({$columnList}) VALUES " . implode(', ', $valueParts);
    ...
    public function getAggregates(string $table, array $aggregates, array $where = []): array
    {
        try {
            ...
            if (isset($aggregates['sum_field'])) {
                $field = $aggregates['sum_field'];
                $selectParts[] = "SUM({$field}) as total_sum";
            }
            if (isset($aggregates['avg_field'])) {
                $field = $aggregates['avg_field'];
                $selectParts[] = "AVG({$field}) as average";
            }
            if (isset($aggregates['max_field'])) {
                $field = $aggregates['max_field'];
                $selectParts[] = "MAX({$field}) as maximum";
            }
            if (isset($aggregates['min_field'])) {
                $field = $aggregates['min_field'];
                $selectParts[] = "MIN({$field}) as minimum";
            }
    ...
    public function bulkUpdateWithCase(string $table, string $idColumn, array $updates): array
    {
        ...
        try {
            ...
            foreach ($columns as $column) {
                $caseStatement = "CASE {$idColumn}";
                foreach ($updates as $id => $record) {
                    $caseStatement .= " WHEN {$id} THEN ?";
                }
                $caseStatement .= " END";
                $setClauses[] = "{$column} = {$caseStatement}";
            }
