...
trait WithdrawalHelperTrait
{
    ...
    public function requestFromUser(int $userId, array $payload): array
    {
        ...
        // کنترل تکرار تراکنش
        $check = $idempotencyService->check($idempotencyKey, $userId, 'withdrawal_request', [
        ...
        try {
            ...
            $failResult = ['success' => false, 'message' => $e->getMessage()];
            ...
            $failResult = ['success' => false, 'message' => 'خطای سیستمی رخ داد'];
            $idempotencyService->fail($idempotencyKey, $failResult, $userId);
            return $failResult;
        }
    }
