...
trait WalletHelperTrait
{
    ...
    public function transfer(int $fromUserId, int $toUserId, string $amount, string $currency = 'irt', string $description = ''): ?object
    {
        ...
        try {
            ...
            if ($fromTransaction && $transaction) {
                ...
                $this->recordWalletOutbox('wallet.transfer.completed', $fromUserId, $fromTransaction->transaction_id, [
                    'to_user_id' => $toUserId,
                    'amount' => $amount,
                    'currency' => $currency,
                ]);
            ...
            $this->invalidateWalletCaches($fromUserId);
            $this->invalidateWalletCaches($toUserId);
        ...
        try {
            return $this->lockService->synchronized("wallet:mut:{$userId}", function() use ($userId, $transaction, $transactionId, $reason) {
                ...
                try {
                    ...
                    $this->recordWalletOutbox('wallet.transaction.reversed', $userId, $transactionId, [
                        'amount' => $amount,
                        'currency' => $currency,
                    ...
                    $this->auditTrail->record('wallet.transaction_reversed', $userId, [
                        'original_transaction_id' => $transactionId,
                        'amount'                  => $amount,
                        'currency'                => $currency,
                        'balance_before'          => $currentBalance,
    ...
    private function recordWalletOutbox(string $eventType, int $userId, mixed $transactionId, array $payload): void
    {
        if (!empty($this->outbox)) {
            $this->outbox->record('wallet', $userId, $eventType, array_merge($payload, [
                'transaction_id' => $transactionId
            ]));
        }
    }
