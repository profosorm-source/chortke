...
class CreateManualDepositJob
{
    ...
    public function __construct(
        \App\Services\Shared\IdempotencyKey $idempotency,
        \Core\RateLimiter $rateLimiter,
        \App\Contracts\LoggerInterface $logger,
        \App\Services\UploadService $uploadService,
        \App\Models\BankCard $bankCardModel,
        \App\Services\Financial\CurrencyService $currencyService,
    ...
    public function handle(int $userId, array $data, ?string $receiptPath): array
    {
        ...
        if (!$this->rateLimiter->attempt("manual_deposit_create:{$userId}", 3, 600)) {
            ...
            return ['success' => false, 'message' => 'تعداد درخواست‌های شما بیش از حد مجاز است. لطفاً ۱۰ دقیقه دیگر تلاش کنید.'];
        }
        ...
        $result = $this->idempotency->executeWithTransaction(
            'manual_deposit_create',
            $userId,
            $payload,
            function () use ($userId, $card, $amount, $receiptPath, $receiptHash, $tracking, $dateStr, $timeStr, $validated) {
