...
class SocialTaskAnalyticsModel extends Model
{
    ...
    {
        $row = $this->db->fetch("SELECT reward FROM social_ads WHERE status = 'active' ORDER BY reward LIMIT 1 OFFSET (SELECT COUNT(*) / 2 FROM social_ads WHERE status = 'active')");
        return (float)($row->reward ?? 0);
    }
}
