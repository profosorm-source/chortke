...
class CustomTaskAnalyticsModel extends Model
{
    ...
    public function analytics_recordView(int $taskId): void
    {
        ...
        $this->db->prepare("UPDATE custom_tasks SET view_count = view_count + 1 WHERE id = ?")->execute([$taskId]);
    }
    ...
    public function analytics_getTrendingTasks(int $limit = 10): array
    {
        $limit = \max(1, $limit);
        $stmt = $this->db->prepare(
            "SELECT id, title, creator_id, currency, view_count, total_budget, spent_budget, completed_count, pending_count
             FROM custom_tasks
    ...
    public function recordTaskView(int $taskId): bool
    {
        ...
        $stmt = $this->db->prepare("UPDATE custom_tasks SET view_count = view_count + 1 WHERE id = ?");
        $stmt->execute([$taskId]);
